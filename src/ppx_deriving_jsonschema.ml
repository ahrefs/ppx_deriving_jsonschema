...
let deriving =
  object (self)
    method name = deriver_name

    method schema_of_core_type ~(config : Attrs.config) ?(recursive_types = []) core_type =
      let loc = core_type.ptyp_loc in
      match core_type with
      | [%type: int] | [%type: int32] | [%type: nativeint] | [%type: int64] -> Schema.type_def ~loc "integer", false
      ...
      | [%type: [%t? t] option] ->
        let s, is_rec = self#schema_of_core_type ~config ~recursive_types t in
        Schema.nullable ~loc s, is_rec
      | [%type: [%t? t] ref] -> self#schema_of_core_type ~config ~recursive_types t
      | [%type: [%t? t] list] | [%type: [%t? t] array] ->
        let t, is_rec = self#schema_of_core_type ~config ~recursive_types t in
      ...
      | Ptyp_constr (id, args) ->
        ...
        | _ ->
          let results = List.map (self#schema_of_core_type ~config ~recursive_types) args in
      ...
      | Ptyp_variant (row_fields, _, _) -> self#schema_of_poly_variant ~loc ~config ~recursive_types row_fields
      | _ ->
    ...
    method schema_of_record ~loc ~(config : Attrs.config) ?(recursive_types = []) fields allow_extra_fields =
        ...
        List.fold_left
          (fun (fields, required, is_rec) ({ pld_name; pld_type; pld_loc = _loc; _ } as field) ->
            ...
            let type_def, field_rec =
              ...
              match pld_type with
              | [%type: [%t? inner] option] ->
                let s, r = self#schema_of_core_type ~config ~recursive_types inner in
                Schema.nullable ~loc s, r
              | _ -> self#schema_of_core_type ~config ~recursive_types pld_type
            in
    ...
    method schema_of_variants ~loc ~(config : Attrs.config) ?(recursive_types = []) variants =
      let variants, is_rec =
        List.fold_left
          (fun (variants, is_rec) ({ pcd_args; pcd_name = { txt = name; _ }; _ } as var) ->
            ...
            | Pcstr_record label_declarations ->
              ...
              let obj_schema, obj_rec =
                self#schema_of_record ~loc ~config ~recursive_types label_declarations allow_extra_fields
              in
              `Tag (name, [ obj_schema ]) :: variants, is_rec || obj_rec
            | Pcstr_tuple typs ->
              let results = List.map (self#schema_of_core_type ~config ~recursive_types) typs in
    ...
    (* Returns (schema_expression, is_recursive) *)
    method schema_of_poly_variant ~loc ~(config : Attrs.config) ?(recursive_types = []) row_fields =
      let constrs, is_rec =
        List.fold_left
          (fun (constrs, is_rec) row_field ->
            ...
            | Rtag (name, false, [ typ ]) ->
              ...
              let raw_typs =
                ...
                | _ -> [ typ ]
              in
              let results = List.map (self#schema_of_core_type ~config ~recursive_types) raw_typs in
            ...
            | Rinherit core_type ->
              let typ, typ_rec = self#schema_of_core_type ~config ~recursive_types core_type in
    ...
    method schema_of_type_decl ~loc ~(config : Attrs.config) ~recursive_types type_decl =
      ...
      | Ptype_variant variants ->
        let schema, is_rec, params_prefix = self#schema_of_variants ~loc ~config ~recursive_types variants in
      ...
      | Ptype_abstract ->
        ...
        | Some core_type ->
          let schema, is_rec = self#schema_of_core_type ~config ~recursive_types core_type in
          type_name, schema, is_rec, params, ""
        | None ->
    ...
    method str_type_decl ~ctxt ast flag_variant_as_string flag_polymorphic_variant_tuple =
      ...
      | _, [ type_decl ] ->
        ...
        let _, raw_schema, is_rec, params, params_prefix =
          self#schema_of_type_decl ~loc ~config ~recursive_types:[ type_name ] type_decl
        in
        let schema =
          if is_rec then
            [%expr
              let ppx_eds = ref [] in
              [%e self#apply_defs ~loc (`Rec (type_name, [ type_name, raw_schema ]))]]
          else
            [%expr
              let ppx_eds = ref [] in
              [%e self#apply_defs ~loc (`NonRec raw_schema)]]
        in
      ...
      | _, type_decls when List.length type_decls > 1 ->
        ...
        let raw_results = List.map (self#schema_of_type_decl ~loc ~config ~recursive_types) type_decls in
            ...
            (fun (name, _, _, params, prefix) ->
              let schema =
                wrap_type_params ~loc ~prefix params
                  [%expr
                    let ppx_eds = ref [] in
                    [%e self#apply_defs ~loc (`Rec (name, defs))]]
              in
            ...
            (fun (name, raw, _, params, prefix) ->
              let schema =
                wrap_type_params ~loc ~prefix params
                  [%expr
                    let ppx_eds = ref [] in
                    [%e self#apply_defs ~loc (`NonRec raw)]]
              in
              create_value ~loc name schema)
            raw_results
      | _, _ -> [%str [%ocaml.error "ppx_deriving_jsonschema: unsupported type"]]

    method sig_type_decl ~ctxt ast _flag_variant_as_string _flag_polymorphic_variant_tuple =
...
let _ : Deriving.t =
  Deriving.add deriving#name
    ~str_type_decl:(Deriving.Generator.V2.make ~attributes:Attrs.attributes (Attrs.args ()) deriving#str_type_decl)
    ~sig_type_decl:(Deriving.Generator.V2.make ~attributes:Attrs.attributes (Attrs.args ()) deriving#sig_type_decl)
