          ...
          [("anyOf",
             (`List
                ...
                (match poly_kind_jsonschema with
                 | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
                     `Assoc (("$id", (`String "file://shared/cases.ml:44"))
                       ::
                       (Stdlib.List.filter
                          (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                          pairs))
            ...
            (`Assoc
               ...
               ("kind_f",
                 ((match kind_jsonschema with
                   | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
                       `Assoc (("$id", (`String "file://shared/cases.ml:49"))
                         ::
                         (Stdlib.List.filter
                            (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                            pairs))
  ...
  struct
    let int_tree_jsonschema =
      ...
      let ppx_result =
        ...
        | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
            `Assoc (("$id", (`String "file://shared/cases.ml:117")) ::
              (Stdlib.List.filter
      ...
      let ppx_result =
        list_jsonschema
          (match event_jsonschema with
           | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
               `Assoc (("$id", (`String "file://shared/cases.ml:118")) ::
                 (Stdlib.List.filter
                    (fun (k, _) -> not (Stdlib.String.equal k "$id")) pairs))
           | other -> other) in
      match !ppx_eds with
      ...
      let ppx_result =
        list_jsonschema
          (list_jsonschema
             (match event_jsonschema with
              | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
                  `Assoc (("$id", (`String "file://shared/cases.ml:119")) ::
                    (Stdlib.List.filter
                       (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                       pairs))
              | other -> other)) in
        ...
        `Assoc
          ...
          ("prefixItems",
            (`List
               [(match event_jsonschema with
                 | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
                     `Assoc (("$id", (`String "file://shared/cases.ml:120"))
                       ::
                       (Stdlib.List.filter
                          (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                          pairs))
      ...
      let ppx_result =
        list_jsonschema
          (match event_comment_jsonschema with
           | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
               `Assoc (("$id", (`String "file://shared/cases.ml:121")) ::
                 (Stdlib.List.filter
                    (fun (k, _) -> not (Stdlib.String.equal k "$id")) pairs))
           | other -> other) in
      match !ppx_eds with
               ...
               (`List
                  [(match event_jsonschema with
                    | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs
                        ->
                        `Assoc
                          (("$id", (`String "file://shared/cases.ml:122")) ::
                          (Stdlib.List.filter
                             (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                             pairs))
                    | other -> other);
      ...
      let ppx_result =
        array_jsonschema
          (match events_jsonschema with
           | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
               `Assoc (("$id", (`String "file://shared/cases.ml:123")) ::
                 (Stdlib.List.filter
                    (fun (k, _) -> not (Stdlib.String.equal k "$id")) pairs))
           | other -> other) in
      match !ppx_eds with
               ...
               [("m",
                  ((match Mod1.m_1_jsonschema with
                    | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs
                        ->
                        `Assoc
                          (("$id", (`String "file://shared/cases.ml:126")) ::
                          (Stdlib.List.filter
                             (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                             pairs))
                    | other -> other)))]));
               ...
               [("obj2",
                  ((match obj2_jsonschema with
                    | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs
                        ->
                        `Assoc
                          (("$id", (`String "file://shared/cases.ml:180")) ::
                          (Stdlib.List.filter
                             (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                             pairs))
                    | other -> other)))]));
  ...
  struct
    let string_link_traffic_jsonschema =
      ...
      let ppx_result =
        ...
        | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
            `Assoc (("$id", (`String "file://shared/cases.ml:193")) ::
              (Stdlib.List.filter
                 (fun (k, _) -> not (Stdlib.String.equal k "$id")) pairs))
        | other -> other in
      match !ppx_eds with
  ...
  struct
    let either_alias_jsonschema a b =
      ...
      let ppx_result =
        ...
        | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
            `Assoc (("$id", (`String "file://shared/cases.ml:209")) ::
              (Stdlib.List.filter
                 (fun (k, _) -> not (Stdlib.String.equal k "$id")) pairs))
        | other -> other in
      match !ppx_eds with
                  ...
                  (option_jsonschema
                     (match composing_type_jsonschema with
                      | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs
                          ->
                          `Assoc
                            (("$id", (`String "file://shared/cases.ml:315"))
                            ::
                            (Stdlib.List.filter
                               (fun (k, _) ->
            ...
            (`Assoc
               ...
               ("a",
                 ((match self_ref_jsonschema with
                   | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
                       `Assoc
                         (("$id", (`String "file://shared/cases.ml:333")) ::
                         (Stdlib.List.filter
                            (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                            pairs))
                   | other -> other)))]));
                     ...
                     (`List
                        ...
                        (match filter_jsonschema atom group_atom with
                         | `Assoc pairs when
                             ...
                             `Assoc
                               (("$id",
                                  (`String "file://shared/cases.ml:342"))
                               ::
                               (Stdlib.List.filter
                                  (fun (k, _) ->
                                     not (Stdlib.String.equal k "$id")) pairs))
               ...
               [("range",
                  ((match match range_jsonschema int_jsonschema with
                          | `Assoc pairs when
                              ...
                              `Assoc
                                (("$id",
                                   (`String "file://shared/cases.ml:402"))
                                ::
                                (Stdlib.List.filter
                                   (fun (k, _) ->
               ...
               ("record",
                 ((match match record_for_default_jsonschema with
                         | `Assoc pairs when
                             ...
                             `Assoc
                               (("$id",
                                  (`String "file://shared/cases.ml:399"))
                               ::
                               (Stdlib.List.filter
                                  (fun (k, _) ->
                                     not (Stdlib.String.equal k "$id")) pairs))
               ...
               ("variant",
                 ((match match variant_for_default_jsonschema with
                         | `Assoc pairs when
                             ...
                             `Assoc
                               (("$id",
                                  (`String "file://shared/cases.ml:398"))
                               ::
                               (Stdlib.List.filter
                                  (fun (k, _) ->
                                     not (Stdlib.String.equal k "$id")) pairs))
               ...
               [("status",
                  ((match match Status.t_jsonschema with
                          | `Assoc pairs when
                              ...
                              `Assoc
                                (("$id",
                                   (`String "file://shared/cases.ml:412"))
                                ::
                                (Stdlib.List.filter
                                   (fun (k, _) ->
                                      not (Stdlib.String.equal k "$id"))
               ...
               [("inner",
                  ((match match inner_with_option_field_jsonschema with
                          | `Assoc pairs when
                              ...
                              `Assoc
                                (("$id",
                                   (`String "file://shared/cases.ml:421"))
                                ::
                                (Stdlib.List.filter
                                   (fun (k, _) ->
                                      not (Stdlib.String.equal k "$id"))
      ...
      struct
        let non_rec_using_wrapper_with_shadowed_stdlib_jsonschema =
          ...
          let ppx_result =
            ...
            | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
                `Assoc (("$id", (`String "file://shared/cases.ml:468")) ::
                  (Stdlib.List.filter
                     (fun (k, _) -> not (Stdlib.String.equal k "$id")) pairs))
            | other -> other in
          match !ppx_eds with
          ...
          struct
            let foo_jsonschema =
              ...
              let ppx_result =
                ...
                | `Assoc pairs when Stdlib.List.mem_assoc "$defs" pairs ->
                    `Assoc (("$id", (`String "file://shared/cases.ml:478"))
                      ::
                      (Stdlib.List.filter
                         (fun (k, _) -> not (Stdlib.String.equal k "$id"))
                         pairs))
